...
class HTTPRedirectHandler(BaseHTTPRequestHandler):
    ...
    https_port = 8125  # Will be set before starting
...
def start_http_redirect_server(http_port: int, https_port: int, log: logging.Logger):
    """Start HTTP server that redirects to HTTPS in a background thread."""
    HTTPRedirectHandler.https_port = https_port
    
    def run_redirect_server():
        try:
            server = HTTPServer(('0.0.0.0', http_port), HTTPRedirectHandler)
            log.info(f"HTTP redirect server on port {http_port} → HTTPS port {https_port}")
            server.serve_forever()
        except Exception as e:
            log.warning(f"HTTP redirect server failed: {e}")
    
    thread = threading.Thread(target=run_redirect_server, daemon=True)
    thread.start()
    return thread
...
def bluetooth_scan():
    ...
    try:
        ...
        # Power on and set pairable BEFORE creating the Popen process
        subprocess.run(["bluetoothctl", "power", "on"], capture_output=True, timeout=5)
        time.sleep(0.5)
        ...
        # Also get devices from a fresh bluetoothctl call (more reliable)
        try:
            device_result = subprocess.run(
                ["bluetoothctl", "devices"],
                capture_output=True, text=True, timeout=5
            )
            device_output = device_result.stdout
            log.info(f"Separate devices query: {device_output}")
        except Exception as dev_err:
            log.warning(f"Separate device query failed: {dev_err}")
            device_output = stdout or ""
        ...
        for line in combined_output.split('\n'):
            # Handle both "Device XX:XX:XX..." and "[NEW] Device XX:XX:XX..." formats
            if "Device " in line:
                # Extract the device part
                device_idx = line.find("Device ")
                if device_idx >= 0:
                    device_part = line[device_idx:]
                    parts = device_part.split(" ", 2)
                    if len(parts) >= 3:
                        mac = parts[1].upper()
                        name = parts[2].strip()
                        
                        # Skip duplicates and already paired
                        if mac in seen_macs or mac in paired_devices:
                            continue
                        seen_macs.add(mac)
...
def bluetooth_connect(mac: str):
    ...
    try:
        ...
        if "Device" not in info_result.stdout:
            return jsonify({"success": False, "error": f"Device {mac} not found. Pair first."})
        
        if "Paired: no" in info_result.stdout:
            return jsonify({"success": False, "error": f"Device {mac} not paired. Pair first."})
        ...
        if "Connection successful" in connect_result.stdout:
        ...
        elif "Failed to connect" in connect_result.stdout:
            return jsonify({
                "success": False, 
                "error": "Failed to connect. Make sure the remote is awake (press any button) and not connected to another device."
            })
        else:
            ...
            connected = "Connected: yes" in info_after.stdout
            return jsonify({
                "success": connected,
                "message": f"{'Connected' if connected else 'Connection unclear'} to {mac}",
                "connected": connected,
                "output": connect_result.stdout
            })
            
    except subprocess.TimeoutExpired:
        return jsonify({"success": False, "error": "Connection timed out. Make sure the remote is awake."})
    except Exception as e:
...
def api_bluetooth_control():
    ...
    try:
        if action == "status":
            ...
            # Wait and power on (use top-level time import)
            time.sleep(2)
            subprocess.run(["bluetoothctl", "power", "on"], capture_output=True, timeout=5)
...
def main():
    ...
    if ssl_enabled:
        ...
        if ssl_context:
            ...
            http_port = web.get("http_redirect_port", 8080)
            start_http_redirect_server(http_port, port, log)
        else:
